...
<%
	try {
		if( "POST".equals(request.getMethod())  && session.getAttribute("key") != null)
		{
			...
			PreparedStatement ps = conn.prepareStatement("select * from student where email = ? and otp = ?");
			ps.setString(1, user);
			ps.setString(2, uid);
			ResultSet rs = ps.executeQuery();
			if(rs.next())
			{
				ps = conn.prepareStatement("update student set pass = ?,otp = '-1',otpTime = -1 where email = ?");
