...
<%
	try {
		if( "POST".equals(request.getMethod())  && session.getAttribute("key") != null)
		{
			...
			if(rs.next())
			{
				ps = conn.prepareStatement("update admin set name = ?,pass = ?,otp = '-1',otpTime = -1,verified = 1 where email = ?");
				ps.setString(1, name);
				ps.setString(2, generateStorngPasswordHash(pass));
				ps.setString(3, user);
				ps.executeUpdate();
				alert = "Your account was setup successfully. Login now to continue...";
